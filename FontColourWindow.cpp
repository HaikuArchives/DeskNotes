...
FontColourWindow::FontColourWindow(
	...
	:
	BWindow(rect, "DeskNotes", B_TITLED_WINDOW,
	...
	messenger(*msg)
{
	...
	initial->FindData(
		"background_colour", B_RGB_COLOR_TYPE, &dataPointer, &dataSize);
	background = (*(rgb_color*) dataPointer);
	originalBackground = background;

	initial->FindData(
		"foreground_colour", B_RGB_COLOR_TYPE, &dataPointer, &dataSize);
	foreground = (*(rgb_color*) dataPointer);
	originalForeground = foreground;

	initial->FindData(
		"font_family_name", B_STRING_TYPE, &dataPointer, &dataSize);
	...
	backgroundColourItem
		= new BMenuItem("Background color", new BMessage(DN_COLOUR_MENU));
	foregroundColourItem
		= new BMenuItem("Text color", new BMessage(DN_COLOUR_MENU));
	...
	colourControl = new BColorControl(B_ORIGIN, B_CELLS_32x8, 8.0,
		"Colour Selector", new BMessage(DN_COLOUR_CHANGE));
	colourControl->SetValue(background);

	defaultsButton = new BButton(
		"Defaults", "Defaults", new BMessage(DN_PROPERTIES_DEFAULTS));
	// Add the revert button, taking away the border added earlier.
	revertButton
		= new BButton("Revert", "Revert", new BMessage(DN_PROPERTIES_REVERT));
...
bool
FontColourWindow::IsDefaultsColor()
{
...
bool
FontColourWindow::IsRevertableColor()
{
	return background != originalBackground || foreground != originalForeground;
}


void
FontColourWindow::_CheckButtons()
{
...
void
FontColourWindow::_UpdateColorControl()
{
...
void
FontColourWindow::MessageReceived(BMessage* msg)
{
	if (msg->WasDropped()) {
		...
		if (msg->FindData(
			...
			message.AddData("background_colour", B_RGB_COLOR_TYPE, &background,
				sizeof(rgb_color));
			message.AddData("foreground_colour", B_RGB_COLOR_TYPE, &foreground,
				sizeof(rgb_color));
	...
	switch (msg->what) {
		...
		case DN_PROPERTIES_DEFAULTS:
		{
			...
			foreground = kDefaultForegroundColor;
			defaultSettings.ReplaceData("background_colour", B_RGB_COLOR_TYPE,
				&background, sizeof(rgb_color));
			defaultSettings.ReplaceData("foreground_colour", B_RGB_COLOR_TYPE,
				&foreground, sizeof(rgb_color));
...
bool
FontColourWindow::QuitRequested()
{
...
void
FontColourWindow::CalculateWindowFrame(BRect* windowFrame, BRect parentFrame)
{
	BRect screenSize;
