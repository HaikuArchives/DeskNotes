	...
	BView(data)
{
	...
	data->FindData(
		"foreground_colour", B_RGB_COLOR_TYPE, (const void**) &fregrnd, &size);
...
DeskNoteView::Archive(BMessage* data, bool deep) const
{
	...
	data->AddData(
		"foreground_colour", B_RGB_COLOR_TYPE, &foreground, sizeof(rgb_color));
...
DeskNoteView::MessageReceived(BMessage* msg)
{
	...
	switch (msg->what) {
		...
		case DN_LAUNCH:
			...
			break;
		case DN_CHG_FNT_CLR:
			msg->FindData("background_colour", B_RGB_COLOR_TYPE,
				(const void**) &bckgrnd, &size);
			msg->FindData("foreground_colour", B_RGB_COLOR_TYPE,
				(const void**) &fregrnd, &size);
			background = *bckgrnd;
			foreground = *fregrnd;
			CascadeFontAndColour();
			break;
		case DN_FNT_CLR:
			if (propertiesWindow == NULL) {
				...
				orginalSettings->AddString(
					"title", "Select colors for this note");
				orginalSettings->AddString("example", aboutText);
				orginalSettings->AddData("background_colour", B_RGB_COLOR_TYPE,
					&background, sizeof(rgb_color));
				orginalSettings->AddData("foreground_colour", B_RGB_COLOR_TYPE,
					&foreground, sizeof(rgb_color));
				orginalSettings->AddData("font_family_name", B_STRING_TYPE,
					&currentFamily, sizeof(font_family));
				orginalSettings->AddData(
					"font_face", B_UINT16_TYPE, &currentFace, sizeof(uint16));

				propertiesWindow = new FontColourWindow(
					windSize, messenger, orginalSettings);
				delete messenger;
...
void
DeskNoteView::CascadeFontAndColour(void)
{
	BFont font;
...
DeskNoteView::RestoreNote(BMessage* msg)
{
	...
	if (msg->FindData("foreground_colour", B_RGB_COLOR_TYPE,
			(const void**) &fregrnd, &size)
		== B_OK) {
		foreground = *fregrnd;
	}
...
void
DeskNoteView::DetachedFromWindow()
{
	const rgb_color* bckgrnd;
	const rgb_color* fregrnd;
	ssize_t size;
	...
	if (propertiesWindow != NULL) {
		...
		orginalSettings->FindData("background_colour", B_RGB_COLOR_TYPE,
			(const void**) &bckgrnd, &size);
		orginalSettings->FindData("foreground_colour", B_RGB_COLOR_TYPE,
			(const void**) &fregrnd, &size);
		background = *bckgrnd;
		foreground = *fregrnd;
		CascadeFontAndColour();
